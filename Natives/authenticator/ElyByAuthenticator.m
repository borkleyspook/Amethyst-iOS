...
// URL to download authlib-injector
...
- (void)downloadAuthlibInjector:(void (^)(BOOL success, NSError *error))completion {
    ...
    // Download the authlib-injector file
    ...
    if ([self isAuthlibInjectorDownloaded]) {
        ...
        [self downloadAuthlibInjector:^(BOOL success, NSError *error) {
            if (!success) {
                ...
                NSString *errorMessage = [NSString stringWithFormat:@"Failed to load authlib-injector for Ely.by authorization: %@", error.localizedDescription];
...
// Method for getting JVM arguments for authlib-injector
...
// Method for handling a repeated login attempt with two-factor authentication
- (void)loginWithTwoFactorToken:(NSString *)token callback:(Callback)callback {
    ...
    // Add the two-factor authentication token to the password
...
//Helper method for sending authentication request
- (void)sendAuthenticateRequest:(NSDictionary *)data manager:(AFHTTPSessionManager *)manager callback:(Callback)callback {
    ...
    [manager POST:authURL parameters:data headers:nil progress:nil success:^(NSURLSessionDataTask *task, NSDictionary *response) {
        @try {
            ...
            if (uuid.length == 32) { // If the UUID has no hyphens
        ...
        if (errorData) {
            @try {
                // Log the contents of errorData for diagnostics 
                ...
                // Check for two-factor authentication (code 401 + specific error message)
                if (response.statusCode == 401 && 
                    ...
                    // Get the ViewController to display the alert 
                    ...
                    // Check for a specific error for invalid credentials 
